...
const Tahsilatlar = () => {
  ...
  const [payments, setPayments] = useState<PaymentDemoRow[] | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [listRes, countRes] = await Promise.all([
        supabase.functions.invoke("sales", { body }),
        supabase.functions.invoke("sales", { body: { action: "payments.counts" } }),
      ]);
      ...
      setPayments((listRes.data?.data as PaymentDemoRow[] | null) ?? []);
  ...
  return (
    <div className="min-h-screen bg-navy-deep px-6 py-10 text-white">
      <div className="mx-auto max-w-5xl">
        ...
        <h1 className="mt-4 font-display text-3xl font-semibold">Tahsilatlar</h1>
        <p className="mt-1 text-white/60">
          Paraşüt'ten senkronize edilen gerçek tahsilatlar (satış faturalarına bağlı ödemeler). Toplam: {totalCount ?? "—"}
        </p>
        <p className="mt-1 text-xs text-white/40">
          Kapsam: yalnızca satış faturalarına bağlı tahsilatlar. Alış faturası/banka masrafı/maaş/vergi ödemeleri bu görünümde yok.
        </p>
        ...
        {!loadError && (
          <div className="mt-6">
            {payments === null ? (
              ...
              <p className="text-white/50">Bu filtrede tahsilat yok.</p>
            ) : (
                ...
                <table className="w-full min-w-[900px] text-left text-sm">
                  <thead className="bg-white/5 text-white/50">
                    <tr>
                      ...
                      <th className="px-4 py-2 font-medium">Bağlı fatura</th>
                    ...
                    {payments.map((p) => (
                      <tr key={p.parasut_id} className="border-t border-white/5">
                        <td className="px-4 py-2">
                          <Link to={`/satislar/tahsilatlar/${p.parasut_id}`} className="hover:text-electric-bright hover:underline">
                            {p.date ?? "—"}
                          </Link>
                        </td>
                        ...
                        <td className="px-4 py-2 text-white/70">
                          {p.payable_parasut_id ? (
                            <Link to={`/satislar/faturalar/${p.payable_parasut_id}`} className="hover:text-electric-bright hover:underline">
                        ...
                        <td className="px-4 py-2 text-white/70">
                          {p.contact_parasut_id ? (
                            <Link to={`/musteriler/${p.contact_parasut_id}`} className="hover:text-electric-bright hover:underline">
                              {p.contact_name ?? `#${p.contact_parasut_id}`}
                            </Link>
                          ) : (
                            "—"
                          )}
